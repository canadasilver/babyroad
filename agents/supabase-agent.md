...
- Google ID Token 로그인은 `supabase.auth.signInWithIdToken` 사용
- Vercel 환경변수
...
- Vercel 환경변수 Value에는 값만 입력한다. `NEXT_PUBLIC_SUPABASE_ANON_KEY=`처럼 키 이름이나 줄바꿈을 포함하지 않는다.
...
- Google 로그인은 Google Identity Services ID Token을 받은 뒤 `signInWithIdToken`으로 세션을 생성한다
- Google ID Token nonce는 Supabase에는 원문 nonce, Google에는 SHA-256 hex hash를 전달한다
- OAuth Client Secret과 `SUPABASE_SERVICE_ROLE_KEY`는 로그인 클라이언트 코드에서 사용하지 않는다
